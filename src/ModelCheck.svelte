<script>
    ...
    const loadCSV = async (model) => {
        const fileName = `modelchecking${models.indexOf(model) + 1}.csv`;
        try {
            const data = await d3.csv(fileName, (d) => ({
                failures: +d.failures,
                observed_target: +d.G3,
                predicted_target: +d.pred,
                source: d.source,
            }));
            csvData = data;
            drawPlot();
...
<div class="feature-card">
    <h3>Model Checking Visualization</h3>
    <div class="model-check-container">
